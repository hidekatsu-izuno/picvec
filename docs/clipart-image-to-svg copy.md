...
VTracerなどのカラートレーサーは初期形状の取得にだけ使い、出力をそのまま完成品にしません。

1. まず透明背景付きPNGから形状をトレースし、重複パス、極小 speckle、ページ全体の背景パスを除く。
2. 色と連結性が近い領域に分け、各領域の形状を独立して確認する。
3. 各領域について、フラット色、線形グラデーション、楕円または円形のラジアルグラデーションを比較する。
4. グラデーションは元画像の色相と明暗幅を保ち、中央の弱い明るさ、安定した本体色、広い内側ベベル程度に抑える。
5. 狭い白帯や強いスポットを新しく作らない。意味のあるハイライト（例: ストレージ上面）は別パスとして再構成する。
6. SVG内へビットマップを埋め込まない。最終成果物はパスとSVGグラデーションだけにする。
7. すべてのアイコンに、合成シルエットの外周だけへ中間灰色 `#66717C` の細い枠を付ける。内部の線や面を個別に囲わない。
8. 枠は形状修正の代わりに使わない。隣接アイコン混入、欠け、境界のずれ、色の消失は先に切り抜きまたは領域形状を直す。

## 4. 画像との比較

各アイコンについて、元画像とSVGを同じピクセルサイズへラスタライズして比較します。

- 白背景と暗背景の両方で96pxと32pxを確認する。
- 透明部分を同じ不透明背景へ合成してから比較する。
- 50%オーバーレイとチェッカー比較を作り、輪郭のずれを確認する。
- シルエットのalpha IoUと、共通部分の色差を測る。
- 人物では髪と顔、あごと首、機器では画面・筐体・ハイライトを個別に確認する。
- 隣接アイコンの混入、欠けた左右端、消えた上面ハイライト、グラデーション幅の縮小を不合格にする。

## 5. SVGDeckへの組み込み

完成したSVGはクリップアート画像として扱います。

```xml
<svg:image id="slide-4--database"
           deck:box="120 210 64 64"
           x="120" y="210" width="64" height="64"
           href="assets/clipart/database.svg"
           aria-label="Database"/>
```

- SVGはスライドへパス展開せず、外部画像リソースとして参照する。
- HTMLではSVG画像をインライン化してよいが、SVGDeck上では非編集画像として扱う。
- PPTXではSVG画像として格納し、編集可能な図形へ変換しない。
- 生成物を安定版へ採用する前に、manifestの全項目、96x96 viewBox、透明背景、XML安全性を検証する。

## 6. 採用前チェックリスト

- [ ] 生成画像は過去のSVGを参照せず、元のグループ画像から作った
- [ ] 各アイコンがセル内で独立し、隣接アイコンが混入していない
- [ ] 重要な左右端、下段の人物、上面ハイライトが欠けていない
- [ ] 96pxと32pxで意味が読み取れる
- [ ] 白背景でも中間灰色の外周で輪郭が確認できる
- [ ] 暗背景で外周が過度に強く見えない
- [ ] SVGにビットマップ、可視テキスト、外部URL、スクリプトがない
- [ ] HTMLとPPTXで非編集画像として出力できる
